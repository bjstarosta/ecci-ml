...
# This requires the nanodash_pos dataset

OUT="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
IN=$( realpath ${OUT}/../nanodash_pos )
...
-i ${IN}/M2_5K01_original.tif \
-o ${OUT}
python ${FILTERS}/filter_sincos.py ${FLAGS} -ks ${KERNEL_SIZE} \
-i ${IN}/M6_5K_original.tif \
-o ${OUT}
python ${FILTERS}/filter_sincos.py ${FLAGS} -ks ${KERNEL_SIZE} \
-i ${IN}/naresh_slide3_original.tif \
-o ${OUT}
...
-o ${OUT}/tmp_exp/
python ${TOOLS} ${FLAGS} augm \
-i ${IN}/M2_5K01_ground.tif \
-o ${OUT}/tmp_ground/
python ${TOOLS} ${FLAGS} augm \
-i ${IN}/M6_5K_ground.png \
-o ${OUT}/tmp_ground/
python ${TOOLS} ${FLAGS} augm \
-i ${IN}/naresh_slide3_ground.tif \
-o ${OUT}/tmp_ground/
